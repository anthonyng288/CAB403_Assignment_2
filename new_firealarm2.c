...
// perams:
//      levels: the number of levels in the carpark
bool tempmonitor(volatile bool fire){
    // store raw data
    fire = false;
...
void open_gates(){
    ...
    for(int i = 0; i < ENTRANCES; i++) {
        while (shm_data->entrances[i].boom.status != 'C') {
            pthread_cond_wait(&shm_data->entrances[i].boom.cond, &shm_data->entrances[i].boom.lock);
        }   
        pthread_mutex_lock(&shm_data->entrances[i].boom.lock);
        shm_data->entrances[i].boom.status = 'R';
        pthread_cond_broadcast(&shm_data->entrances[i].boom.cond);
        pthread_mutex_unlock(&shm_data->entrances[i].boom.lock);
    }
    // for (int i = 0; i < EXITS; i++) {
    //     while (shm_data->entrances[i].boom.status != 'C') {
    //         pthread_cond_wait(&shm_data->exits[i].boom.cond, &shm_data->exits[i].boom.lock);
    //     } 
    //     pthread_mutex_lock(&shm_data->exits[i].boom.lock);
    //     shm_data->exits[i].boom.status = 'R';
    //     pthread_cond_broadcast(&shm_data->exits[i].boom.cond);
    //     pthread_mutex_unlock(&shm_data->exits[i].boom.lock);
    // }
    return;
}

void evacuate_garage(shared_data_t* parking, volatile bool fire){
